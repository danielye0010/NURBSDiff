...
std::vector<torch::Tensor> surf_pre_compute_basis(torch::Tensor u,
  ...
  return {uspan_out, vspan_out, Nu_out, Nv_out};
}
...
torch::Tensor surf_forward(
    ...
    int q,
    int _dimension) {
  auto surface = torch::zeros({ctrl_pts.size(0), u_uv.size(0), v_uv.size(0), _dimension+1});
    ...
    for (int j=0; j<v_uv.size(0); j++)
    {
      for (int i=0; i<u_uv.size(0); i++)
      {
        ...
        for (int l = 0; l<=q; l++)
        {
          ...
          for (int r = 0; r<=p; r++)
          {
            grad_ctrl_pts[k][uspan_uv[i].item<int>() - p + r][vspan_uv[j].item<int>() - q + l] =
              grad_ctrl_pts[k][uspan_uv[i].item<int>() - p + r][vspan_uv[j].item<int>() - q + l] +
              Nu_uv[i][r].item<float>()*grad_temp[l];
          }
        }
      }
    }
